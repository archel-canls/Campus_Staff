...
<head>
    ...
    <title>Register | Campus Staff Management System</title>
...
<body class="bg-slate-50">
                ...
                <form action="{{ route('register') }}" method="POST" enctype="multipart/form-data" class="space-y-10" id="regForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-6 bg-slate-50 rounded-3xl space-y-4 border border-slate-100">
                            <p class="text-[9px] font-black text-cdi-orange uppercase tracking-[0.2em] ml-4">Kontak Utama</p>
                    ...
                    <div class="pt-8">
                        <button type="submit" id="btnSubmit" class="w-full bg-cdi text-white py-5 rounded-[2rem] font-black uppercase italic tracking-widest text-xs shadow-xl shadow-blue-900/20 hover:bg-cdi-orange hover:-translate-y-1 transition-all">
                            <span id="btnText">Selesaikan Pendaftaran</span> <i class="fas fa-check-double ml-2"></i>
                        </button>
                    </div>
                </form>
    ...
    {{-- MODAL OTP --}}
    <div id="otpModal" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm hidden items-center justify-center z-[100] p-4">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl border-4 border-cdi-orange">
            <div class="text-center mb-8">
                <div class="w-20 h-20 bg-orange-100 text-cdi-orange rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white shadow-lg">
                    <i class="fas fa-shield-alt text-3xl"></i>
                </div>
                <h4 class="text-2xl font-black uppercase italic text-cdi">Verifikasi Email</h4>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 px-4">Masukkan 6 digit kode OTP yang dikirim ke email Anda</p>
            </div>

            <input type="text" id="otp_input" maxlength="6" class="w-full text-center tracking-[0.5em] font-black text-3xl py-5 border-2 border-slate-100 rounded-3xl focus:border-cdi-orange focus:ring-0 outline-none mb-6 bg-slate-50" placeholder="000000">

            <button type="button" id="btnVerifyOtp" onclick="finalizeRegistration()" class="w-full bg-cdi text-white py-5 rounded-3xl font-black uppercase italic tracking-widest text-sm hover:bg-cdi-orange hover:-translate-y-1 transition-all shadow-xl shadow-blue-900/20">
                Verifikasi & Daftar
            </button>
            <button type="button" onclick="closeOtpModal()" class="w-full mt-4 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-red-500">Batal</button>
        </div>
    </div>
    ...
    <script>
        ...
        // 2. LOGIKA JALUR PENDAFTARAN (SINKRONISASI DROPDOWN & LABEL)
        ...
        if (jalurSelect) {
            jalurSelect.addEventListener('change', function() {
                ...
                `;
                    statusSelect.innerHTML = `
                    <option value="" disabled selected>Pilih Program Magang</option>
        ...
        // 3. AUTO-GENERATE NIP (12 DIGIT)
        // Format: [YYMM (Sekarang)] + [JK (1/2)] + [YYMM (Lahir)] + [Random 3 Digit]
        function generateNIP() {
            const tglLahirVal = document.getElementById('tanggal_lahir').value;
            const jkVal = document.getElementById('jenis_kelamin').value;

            if (!tglLahirVal || !jkVal) return "";

            const now = new Date();
            // Part 1: Tahun & Bulan Masuk (4 digit)
            const part1 = now.getFullYear().toString().slice(-2) +
                (now.getMonth() + 1).toString().padStart(2, '0');

            // Part 2: Jenis Kelamin (1 digit: L=1, P=2)
            const part2 = (jkVal === 'L') ? "1" : "2";

            // Part 3: Tahun & Bulan Lahir (4 digit)
            const dob = new Date(tglLahirVal);
            const part3 = dob.getFullYear().toString().slice(-2) +
                (dob.getMonth() + 1).toString().padStart(2, '0');

            // Part 4: Unique Random (3 digit)
            const part4 = Math.floor(Math.random() * 900 + 100).toString();

            return part1 + part2 + part3 + part4;
        }

        // 4. LOGIKA SUBMIT & OTP VERIFICATION
        let otpVerified = false;
        const regForm = document.getElementById('regForm');

        if (regForm) {
            regForm.addEventListener('submit', function(e) {
                // Jika OTP sudah diverifikasi, izinkan submit form ke database
                if (otpVerified) return;

                // Hentikan pengiriman form untuk proses verifikasi OTP terlebih dahulu
                e.preventDefault();
                ...
                // Validasi Input Wajib Sebelum OTP
                if (!email || !tglLahir || !jk) {
                    alert('Silakan lengkapi Email, Jenis Kelamin, dan Tanggal Lahir sebelum melanjutkan.');
                    return;
                }
                ...
                // Ubah UI Button menjadi Loading
                const btnSubmit = document.getElementById('btnSubmit');
                const btnText = document.getElementById('btnText');
                btnSubmit.disabled = true;
                btnText.innerText = 'Sedang Mengirim Kode OTP...';
        ...
        function sendOtpRequest(email) {
            fetch("{{ route('otp.send') }}", {
                ...
                .catch(() => {
                    alert('Gangguan koneksi saat mengirim OTP.');
                    resetSubmitButton();
                });
        }
        ...
        function finalizeRegistration() {
            ...
            if (otpInput.length < 6) {
                alert('Harap masukkan 6 digit kode OTP dengan benar.');
                return;
            }
            ...
            fetch("{{ route('otp.verify') }}", {
                ...
                .then(data => {
                    if (data.success) {
                        // --- KRUSIAL: PENGISIAN NIP DILAKUKAN DI SINI ---
                        otpVerified = true;
                        const generatedNip = generateNIP();
                        document.getElementById('nip_auto').value = generatedNip;

                        // Log untuk memastikan di console (opsional)
                        console.log("NIP Berhasil di-generate: " + generatedNip);

                        // Submit form secara programatik ke Controller
                        regForm.submit();
                    } else {
                        alert(data.message || 'Kode OTP salah atau sudah kadaluarsa.');
                        btnVerify.disabled = false;
                        btnVerify.innerText = 'Verifikasi & Daftar';
                    }
                })
                .catch(() => {
        ...
        function resetSubmitButton() {
            ...
            if (btnSubmit) {
                ...
                btnText.innerText = 'Selesaikan Pendaftaran';
            }
        }
    </script>
</body>
